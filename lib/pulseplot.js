...
*/
    ...
    drawCanvas() {
        ...
        // extra info
        if (this.data.time)
            ctx.fillText(`${this.data.time}`, 100, fontY)
        if (this.data.format || this.data.mod)
            ctx.fillText(`${(this.data.format || this.data.mod).toUpperCase()}`, 300, fontY)
        if (this.data.pulses)
            ctx.fillText(`${this.data.pulses.length / 2} pulses`, 330, fontY)
